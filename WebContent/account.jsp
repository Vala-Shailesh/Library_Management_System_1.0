...
<head>
	...
	<meta name="msapplication-square70x70logo" content="./resources/Images/Flavicons/Wiindow/icon-70.png">
     ...
     <!-- script   -->  
     <script type="text/javascript" src="./resources/JS/profileUpdateControl.js" defer></script>
...
<body>
	...
	<%  
		// get current user data using seesion 
		...
		ArrayList<String> userDetails = userData.getUser(user);
				
	%>

     <!-- account container -->
     <div class="container-fluid m-2 mt-5 w-90 box-shadow-1 row justify-content-center p-4 rounded-4 container-center">
          <div class="user-img col-lg-4 col-md-12 col-sm-12">
               <img src="./resources/Images/Main/profile_img.jpg" alt="user image" width="300px" height="300px" class="container-center rounded-circle">
          </div>
          <div class="col-lg-8 col-md-12 col-sm-12" >
               <h1 class="mt-5 align-center"> User Profile </h1>
               <form action="./userupdate" method="post" class="mt-5">
                    <div class="form-item"> 
                         <label for="userid" class=""> User ID : </label>
                         <input type="text" name="userid" id="userid" value="<%= userDetails.get(0) %>"  class="input field" readonly">
                    </div>
                    <div class="form-item" >
                         <label for="username" class=""> User Name : </label>
<%--                           <p class="w-75 border-b-dark-1 inline-block"> <%= userDetails.get(1) %> </p> --%>
                         <input type="text" name="username" id="username" value="<%= userDetails.get(1) %>"  class="input field" required>
                    </div> 
                    <div class="form-item" >
                         <label for="useremail" class=""> User Email : </label>
<%--                           <p class="w-75 border-b-dark-1 inline-block"> <%= userDetails.get(2) %> </p> --%>
                         <input type="email" name="useremail" id="useremail" value="<%= userDetails.get(2) %>" class="input field" required>
                    </div>
                    <div class="form-item" >
                         <label for="password" class=""> User Password :  </label>
<%--                          <p class="w-75 border-b-dark-1 inline-block""> <%= userDetails.get(3) %> </p> --%>
                         <input type="password" name="password" value="<%= userDetails.get(3) %>" class="input field" required>
                    </div>
                    <div class="form-item d-flex justify-content-center">
                         <button type="submit" class="btn btn-success mt-4 m-2 field"> Save </button>
                         <button type="button" class="btn btn-warning mt-4 m-2 enable"> Update </button>
                         <button type="button" class="btn btn-danger mt-4 m-2"> <a href="./logout" class="nav-link logout"> Logout</a></button>
                    </div>
               </form>
          </div>
     </div> 
	 ...
	 <%
		 	// get update status if updated !
			Object updateStatus = request.getAttribute("updateStatus");
	 
	 		String updateStatusBox = "none";
	 		if( updateStatus != null ){
	 			updateStatusBox = "block";
	 		}
	 %>
		
	<!-- popup box for successfully update data -->
     <div class="container register-box box-shadow-1 p-4 rounded-2" style="display : <%= updateStatusBox %>; " >
		<h3 class="text-success"> Data Update Successfully </h3> 
		<hr>
		<p> Your data are updated successfully. Remember for future use. </p>
		<button type="button" class="btn btn-end btn-success registerBox-close-btn w-25"> Ok </button>
	</div>

     <!-- footer -->
     <div class="container-fluid w-100 p-5 box-shadow-1 mt-5 footer">
